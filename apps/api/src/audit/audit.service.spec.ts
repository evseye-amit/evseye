...
describe('AuditService', () => {
  it('returns a tenant-scoped paginated audit log', async () => {
    ...
    await expect(service.list('tenant-a', 2, 200)).resolves.toEqual({
      items: [{ id: 1n }],
    ...
    expect(count).toHaveBeenCalledWith({ where: { tenantId: 'tenant-a' } });
  });
});
