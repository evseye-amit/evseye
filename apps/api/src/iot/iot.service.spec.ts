...
const config = {
  getOrThrow: vi.fn(
    () => 'iot-test-secret-with-at-least-thirty-two-characters',
  ),
};
...
function createService() {
  const prisma = {
    fleet: {
      findFirst: vi.fn().mockResolvedValue({ id: 'fleet-1' }),
      update: vi.fn().mockResolvedValue({ id: 'fleet-1' }),
    },
    ioTDevice: {
      create: vi
        .fn()
...
describe('IotService', () => {
  it('returns a one-time ingestion secret and stores only its hash', async () => {
    ...
    expect(
      prisma.ioTDevice.create.mock.calls[0][0].data.ingestSecretHash,
    ).not.toBe(result.ingestSecret);
  });
  ...
  it('rejects an invalid secret before accepting telemetry', async () => {
    ...
    prisma.ioTDevice.findFirst.mockResolvedValue({
      id: 'device-1',
      clientId: 'client-1',
      ingestSecretHash: '0'.repeat(64),
    });

    await expect(
      service.ingest('device-serial-1', 'incorrect-secret', 'LOCATION', {}),
  ...
  it('derives the fleet identity from authenticated device credentials', async () => {
    ...
    prisma.ioTDevice.findFirst.mockResolvedValue({
      ...registration.device,
      currentFleet: { id: 'fleet-1' },
    });

    await expect(
      service.ingest('device-serial-1', registration.ingestSecret, 'LOCATION', {
        latitude: 19.076,
        longitude: 72.8777,
      }),
