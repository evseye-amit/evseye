...
export default function Home() {
  ...
  const [allocationDetail, setAllocationDetail] = useState<RecordItem | null>(null);
  ...
  async function loadView(nextTab: Tab) {
    ...
    try {
      if (nextTab === "dashboard") setDashboard(await request("/dashboard", {}, token) as Dashboard);
        ...
        const result = await request(`/${nextTab}?${query.toString()}`, {}, token) as { items: RecordItem[]; meta: { page: number; pageSize: number; total: number } };
  ...
  async function openInspection(allocation: RecordItem) {
    const targetType = allocation.status === "DEALLOCATION_INITIATED" ? "POST_DEALLOCATION" : "PRE_ALLOCATION";
    const inspection = ((allocation.inspections as RecordItem[] | undefined) ?? []).find((item) => item.type === targetType);
    if (!inspection?.id) return setError(`This allocation has no ${targetType.toLowerCase().replaceAll("_", " ")} inspection.`);
    setLoading(true); setError("");
    try {
      ...
      setInspectionId(String(inspection.id)); setInspectionType(targetType); setRequirements(photoRequirements);
  ...
  async function completeInspection() {
    ...
    try {
      ...
      setNotice(inspectionType === "POST_DEALLOCATION" ? "Post-deallocation inspection completed. Verify both OTPs to finish." : "Inspection completed. The allocation is ready for activation."); setInspectionId(""); await loadView("allocations");
  ...
  async function openAllocationDetail(allocationId: string) {
    setLoading(true); setError("");
    try {
      const detail = await request(`/allocations/${allocationId}`, {}, token) as RecordItem;
      setAllocationDetail(detail);
      const fleet = detail.fleet as RecordItem;
      setVehicleState(await request(`/fleets/${String(fleet.id)}/current-state`, {}, token) as RecordItem | null);
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load allocation details."); }
    finally { setLoading(false); }
  }

  async function openRiderDetail(riderId: string) {
    setLoading(true); setError("");
    try { setRiderDetail(await request(`/riders/${riderId}`, {}, token) as RecordItem); }
    catch (cause) { setError(cause instanceof Error ? cause.message : "Unable to load rider details."); }
    finally { setLoading(false); }
  }
  ...
  return <main className="app-shell"><aside className="sidebar"><div><p className="eyebrow">EVS EYE</p><h2>Operations</h2></div><nav>{(["dashboard", "fleets", "riders", "allocations"] as Tab[]).map((item) => <button key={item} className={tab === item ? "nav-active" : ""} onClick={() => setTab(item)}>{item}</button>)}</nav><button className="sign-out" onClick={signOut}>Sign out</button></aside>
    <section className="workspace"><header><div><p className="eyebrow">TENANT WORKSPACE</p><h1>{title}</h1></div><div className="header-actions">{tab === "allocations" && <button onClick={() => void openAllocationForm()}>New allocation</button>}<button className="secondary" onClick={() => void loadView(tab)}>Refresh</button></div></header>{tab !== "dashboard" && <form className="list-filters" onSubmit={(event) => { event.preventDefault(); void loadView(tab); }}><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={tab === "fleets" ? "Search vehicle, chassis, OEM" : tab === "riders" ? "Search rider or mobile" : "Filter by fleet or rider ID"} /><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All statuses</option>{(tab === "fleets" ? ["AVAILABLE", "ALLOCATED", "MAINTENANCE", "OFFLINE"] : tab === "riders" ? ["ACTIVE", "PENDING", "BLOCKED"] : ["INSPECTION_PENDING", "OTP_PENDING", "ACTIVE", "DEALLOCATION_INITIATED", "COMPLETED"]).map((status) => <option key={status} value={status}>{status.replaceAll("_", " ")}</option>)}</select><button>Apply</button></form>}{notice && <p className="notice">{notice}</p>}{error && <p className="error">{error}</p>}{loading && <p className="muted">Loading current data…</p>}
      ...
      {allocationDetail && <section className="action-card detail-card"><div><p className="eyebrow">ALLOCATION DETAIL</p><h2>{String((allocationDetail.fleet as RecordItem)?.vehicleNumber)} · {String((allocationDetail.rider as RecordItem)?.name)}</h2><p><Status value={String(allocationDetail.status)} /></p></div><div className="detail-grid"><div><strong>Rider</strong><span>{String((allocationDetail.rider as RecordItem)?.mobile ?? "—")}</span></div><div><strong>Fleet</strong><span>{String((allocationDetail.fleet as RecordItem)?.oem ?? "—")}</span></div><div><strong>Location</strong><span>{vehicleState?.latitude && vehicleState?.longitude ? `${String(vehicleState.latitude)}, ${String(vehicleState.longitude)}` : "No location received"}</span></div><div><strong>Last heartbeat</strong><span>{vehicleState?.lastHeartbeat ? new Date(String(vehicleState.lastHeartbeat)).toLocaleString() : "Not received"}</span></div></div><h3>Inspections</h3><div className="inspection-summary">{((allocationDetail.inspections as RecordItem[]) ?? []).map((inspection) => <span key={String(inspection.id)}><b>{String(inspection.type).replaceAll("_", " ")}</b> <Status value={String(inspection.status)} /></span>)}</div><button className="secondary" onClick={() => setAllocationDetail(null)}>Close detail</button></section>}
      {riderDetail && <section className="action-card detail-card"><p className="eyebrow">RIDER DETAIL</p><h2>{String(riderDetail.name)}</h2><div className="detail-grid"><div><strong>Mobile</strong><span>{String(riderDetail.mobile)}</span></div><div><strong>Status</strong><Status value={String(riderDetail.status)} /></div></div><h3>KYC</h3><div className="inspection-summary">{((riderDetail.kycs as RecordItem[]) ?? []).map((kyc) => <span key={String(kyc.id)}><b>{String(kyc.type)}</b> <Status value={String(kyc.status)} /></span>)}</div><h3>Open allocations</h3><div className="inspection-summary">{((riderDetail.allocations as RecordItem[]) ?? []).map((allocation) => <span key={String(allocation.id)}>{String((allocation.fleet as RecordItem)?.vehicleNumber)} <Status value={String(allocation.status)} /></span>)}</div><button className="secondary" onClick={() => setRiderDetail(null)}>Close detail</button></section>}
